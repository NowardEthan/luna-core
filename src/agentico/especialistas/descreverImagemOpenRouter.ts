...
/**
 * Visão via provider pessoal (BYOK do criador — Claude/Fable ou outro OpenAI-compat).
 *
 * Por padrão a visão da Luna usa OpenRouter (gemini/qwen/mistral). Mas se o cérebro do turno é
 * o provider pessoal, a visão TEM que ir pelo mesmo provider — senão:
 `1)` o OpenRouter cai (chave sem modelo de vídeo, cota estourada, etc), a Luna diz
 *    'não consegui ver' mesmo com Claude/Fable funcionando pro texto.
 `2)` O Claude/Fable pode não saber processar URL do Storage do Firebase (rede
 *    separada). Mandamos a URL primeiro e, se falhar, baixamos o vídeo e mandamos como data URL.
 *
 * OpenAI-compat genérico — funciona com Claude/Fable, OpenRouter-BYOK, OpenAI, Together, Groq etc.
 * O payload segue o schema OpenAI Chat Completions multimodal.
 */
export type PersonalProviderVisao = {
  apiKey: string;
  baseUrl: string;
  model: string;
};

async function chamarPersonalProviderChat(
  provedor: PersonalProviderVisao,
  mensagem: { role: "user"; content: Array<Record<string, unknown>> },
  maxTokens = 1200,
): Promise<Response> {
  const url = provedor.baseUrl.replace(/\/+$/, "") + "/chat/completions";
  return fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
function conteudoMidiaPersonal(ehVideo: boolean, fonte: string): Record<string, unknown> {
  return ehVideo
    ? { type: "video_url", video_url: { url: fonte } }
    : { type: "image_url", image_url: { url: fonte } };
}
...
/**
 * Lê a imagem ou vídeo com o provider pessoal.
 *
 * Ordem de tentativa (espelha o fallback do OpenRouter):
 `1)` URL do Storage direto (o provedor busca o arquivo).
 `2)` Se o vídeo falhar com URL, baixa o vídeo e reenvia como data URL.
 `3)` Se ainda falhar e há frame JPEG, manda o frame como imagem.
 *
 * Devolve texto da descrição. Lança erro com mensagem detalhada se todas as tentativas falharem
 * (o `visaoGemma` traduz pra `VISUAL_ANALYSIS_FAILED` na resposta da Luna).
 */
export async function descreverImagemPersonalProvider(
  entrada: { imagem: AnexoImagemChat; pergunta?: string },
  provedor: PersonalProviderVisao,
): Promise<string> {
  const { imagem, pergunta } = entrada;
  const mime = imagem.mimeType?.trim() || "image/jpeg";
  ...
  const fonte = imagem.url?.trim() || (imagem.imageBase64?.trim() ? `data:${mime};base64,${imagem.imageBase64.trim()}` : null);
  if (!fonte) throw new Error("Anexo sem URL nem base64 — nada para olhar.");
  ...
  const montarMensagem = (midiaUrl: string) => ({
    role: "user" as const,
    content: [
      { type: "text", text: instrucao },
      conteudoMidiaPersonal(ehVideo, midiaUrl),
    ],
  });

  let res = await chamarPersonalProviderChat(provedor, montarMensagem(fonte));
  let motivo = "";

  if (!res.ok && ehVideo && imagem.url?.trim()) {
    const urlOriginal = imagem.url.trim();
    try {
      const dataUri = await videoUrlParaDataUri(urlOriginal, mime);
      const res2 = await chamarPersonalProviderChat(provedor, montarMensagem(dataUri));
      if (res2.ok) {
        res = res2;
      } else {
        const corpo = (await res2.text()).slice(0, 240);
        motivo = `retry base64 falhou (${res2.status}): ${corpo};`;
      }
    } catch (erro) {
      const msg = erro instanceof Error ? erro.message : String(erro);
      motivo += `retry base64 erro: ${msg};`;
    }
  }

  if (!res.ok && ehVideo && imagem.thumbnailUrl?.trim()) {
    const instrucaoFrame = [
      instrucaoBase(false),
      "",
      "O processamento do vídeo inteiro falhou. Este anexo é um frame JPEG extraído do vídeo.",
      "Use o frame para responder ao que for visível, mas deixe claro quando algo dependeria de movimento/áudio ou de outros momentos do vídeo.",
      motivo ? `Falha do vídeo: ${motivo}` : "",
      pergunta?.trim()
        ? `Pergunta específica sobre o vídeo/frame: ${pergunta.trim()}`
        : "Descreva o que aparece no frame e o que isso permite inferir com segurança.",
    ].filter(Boolean).join("\n");
    const mensagemFrame = {
      role: "user" as const,
      content: [
        { type: "text", text: instrucaoFrame },
        conteudoMidiaPersonal(false, imagem.thumbnailUrl.trim()),
      ],
    };
    res = await chamarPersonalProviderChat(provedor, mensagemFrame);
    if (!res.ok) {
      const corpo = (await res.text()).slice(0, 240);
      throw new Error(`frame falhou (${res.status}): ${corpo}; ${motivo}`);
    }
  }

  if (!res.ok) {
    const corpo = (await res.text()).slice(0, 240);
    throw new Error(`visão pessoal falhou (${res.status}): ${corpo}; ${motivo}`);
  }
  ...
  const texto = data.choices?.[0]?.message?.content?.trim();
  if (!texto) throw new Error("O modelo pessoal não devolveu descrição.");
  return texto;
}
