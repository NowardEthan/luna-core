...
const DIRETRIZ_DOCUMENTOS =
  ...
  "COMO ESCREVER o corpo (isto importa — um artefato não é uma mensagem de chat esticada, dá-lhe FORMA): abre com uma frase ou duas que situam o assunto; divide em SEÇÕES com subtítulos `## ` (e `### ` quando precisares de um nível a mais); usa listas com `- ` para itens soltos e `1. ` para passos em ordem; quando for um PLANO ou uma lista de TAREFAS que ele vai executar e ir riscando, usa CHECKLIST — `- [ ] ` uma caixa por tarefa (em vez de `- `) — que ele marca com o dedo no leitor e fica salvo; começa um item com **termo em negrito** quando há um rótulo a destacar; para um aviso/destaque que merece saltar à vista usa um CALLOUT COM SABOR — uma linha `> ` que abre com um marcador `[!sabor]`, onde `sabor` é um de: `dica` 💡, `atencao` ⚠️, `feito` ✅, `duvida` ❓, `fixado` 📌, `info` ℹ️ (ex.: `> [!dica] Guarda o comprovante.` ou `> [!atencao] Isto apaga tudo.`) — escolhe o sabor pelo tom (uma sacada é `dica`, um perigo é `atencao`, algo concluído é `feito`, uma pergunta em aberto é `duvida`, um lembrete a fixar é `fixado`, uma nota neutra é `info`); um `> ` sem marcador continua a ser uma citação normal. Se estás a comparar coisas pelos mesmos campos, uma tabela Markdown (| … | … |) lê muito melhor que um parágrafo. Um traço `---` separa partes grandes; se a divisão merece um nome, usa um DIVISOR ROTULADO `--- Parte 2 ---` (o rótulo vai no meio da linha). NÃO enches de seção por encher — a estrutura serve a leitura, não a enfeita; um bilhete curto continua curto. É a tua voz de sempre, só que organizada para durar e reabrir. " +
...
const DIRETRIZ_ORIENTACAO_ESCRITA =
  ...
  "1) `ler_estrutura` 2) `ler_secao` da seção alvo ou da última (no máx. 1–2 seções — sem ping-pong) " +
  ...
  "Se já leu o suficiente: ESCREVA. Não «ambientar» o livro inteiro no mesmo turno. " +
...
const DIRETRIZ_AUDITORIA_ARTEFATO =
  "AUDITORIA depois de ESCREVER/EDITAR artefato: releia UMA vez (`ler_estrutura` preferível, " +
  "ou `ler_secao` SÓ do pedaço mexido) — sem voltar a alternar capítulos. Confira se as seções " +
  "antigas ainda estão e se o novo entrou no lugar certo. " +
